...
describe("AdminVideoAcademyScreen (Tier 2 detail)", () => {
  ...
  it("carries no access controls — subscription/tier live on the client page (R4, one writer)", async () => {
    renderScreen();
    await screen.findByText("Noor Academy");
    expect(screen.queryByTestId("deactivate")).not.toBeInTheDocument();
    expect(screen.queryByTestId("tier-select")).not.toBeInTheDocument();
    const link = screen.getByText(V.detail.openClient);
    expect(link.closest("a")).toHaveAttribute("href", "/admin/clients/a1");
  });
