 ...
 *
 * The catalogue arrives as a prop: `page.tsx` loads it on the server so the hero's preview card and
 * the course grid are already right on the first frame. The browser fetch below is the fallback for
 * the one case the server could not cover — the API was unreachable when the page was rendered.
 *
...
export function SiteHome({
  /** What the server already loaded; null only when that fetch failed. */
  initialCourses,
}: {
  initialCourses: LearnCourseCard[] | null;
}) {
  const t = useTranslations("learn");
  const { academy, site, commerce, openRedeem } = useLearn();
  const cta = useHeroCta();
  const [courses, setCourses] = useState<LearnCourseCard[] | null>(
    initialCourses,
  );
