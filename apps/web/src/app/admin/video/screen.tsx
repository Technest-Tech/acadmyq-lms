...
export function AdminVideoScreen() {
  ...
  return (
    <div className="w-full space-y-6 p-4 sm:p-6">
      ...
      <section className="bg-card rounded-2xl border shadow-sm ring-1 ring-foreground/[0.04]">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b px-5 py-4">
          ...
          <button
            ...
            className="bg-primary text-primary-foreground inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold transition-opacity hover:opacity-90"
          >
      ...
      {adding && (
        <AddAcademyModal
          enabledIds={new Set((usage?.academies ?? []).filter((a) => a.video_enabled).map((a) => a.academy_id))}
