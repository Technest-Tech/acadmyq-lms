...
/**
 * Checkout (docs/lms/10 §3) — the «اشترِ الكورس» screen, and the one that sells a book.
 *
 ...
 *
 * ONE screen serves both a course and a book (docs/lms/11). Everything a buyer does here is the
 * same for either — it is the same order, the same receipt, the same queue — so `kind` only picks
 * which endpoint to call and where "you already own this" sends them. Keeping two copies of this
 * flow would guarantee they drift on exactly the details (consent, resumability) that matter.
 */
...
export function CheckoutScreen({ kind }: { kind: "course" | "book" }) {
  ...
  // Where the item lives on the site, and where "you already own this" should go.
  const itemHref = href(isBook ? `/b/${slug}` : `/c/${slug}`);
  const ownedHref = href(isBook ? `/b/${slug}` : `/watch/${slug}`);
  ...
  useEffect(() => {
    ...
    const load = isBook
      ? learnBookCheckout(academy, slug).then(
          // The two payloads differ in exactly two key names; normalising here keeps the whole
          // screen below written against one shape.
          (d): LearnCheckout => ({ ...d, already_enrolled: d.already_owned, course: d.product }),
        )
  ...
  async function place() {
    ...
    setError(null);
    try {
      const body = { payment_method_id: methodId, accept_terms: true } as const;
      const r = isBook
        ? await learnPlaceBookOrder(academy, slug, body)
        : await learnPlaceOrder(academy, slug, body);
  ...
  if (error && !data) {
    return (
      <Container className="py-24 text-center">
        ...
        <CtaButton variant="outline" className="mt-6" href={itemHref}>
          {t("course.back")}
        </CtaButton>
      </Container>
    );
  ...
  // Already owns it — send them to the player (or the download) rather than selling it twice.
  if (data.already_enrolled) {
    return (
      <Container className="py-24 text-center">
        <CheckCircle2 className="mx-auto size-10 text-emerald-600" />
        <h1 className="mt-4 text-2xl font-bold">{t("checkout.alreadyOwned")}</h1>
        <p className="text-muted-foreground mt-2 text-sm" dir="auto">
          {data.course.title}
        </p>
        <CtaButton className="mt-6" href={ownedHref}>
          {isBook ? (
            <Download className="size-4" aria-hidden />
          ) : (
            <PlayCircle className="size-4" aria-hidden />
          )}
          {isBook ? t("books.download") : t("course.continue")}
        </CtaButton>
      </Container>
    );
  }
