...
it('TC-7.30: PER_GUARDIAN, mixed currencies (EGP + SAR) → falls back to per-child invoices', function () {
    $studentB = $this->createStudent($this->academy, $this->guardian);
    ...
    $this->asAcademy($this->academy);
    // Mixed currencies → no single guardian invoice; each student has their own.
    ...
    // Each student ends up on a per-student invoice (student_id set).
    ...
    // No single multi-currency guardian invoice.
    expect($guardianInvoices)->toBe(0);
    // Two per-student invoices (one per child).
    expect($studentInvoices)->toBe(2);
});
